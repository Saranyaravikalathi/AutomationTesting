...
test('Create Leads', async ({ page }) => {
    ...
    // Fill in the form
    await fillInput(xpaths.CreateLeads.company, 'Neartekpod');
    await clickElement(xpaths.CreateLeads.FnDrpdown);
    await clickElement(xpaths.CreateLeads.FnDrpdownmenu);
    await fillInput(xpaths.CreateLeads.firstName, 'Saranya');

    console.log('Lead creation form filled successfully!');
